...
void search()
{
    ...
    if (permutation.size() == A_p.size())
    {
        ...
        for(long long  i = 0; i < m ; i++){
            if(x[i] == y[i]){
                sum += permutation[x[i]]% 1000000007;
            } else {
                sum += (permutation[x[i]] + permutation[y[i]])% 1000000007;
            }
        }
    ...
    {
        for (long long  i = 0; i < A_p.size(); i++)
        {
            if (chosen[i])
                continue;
            chosen[i] = true;
            permutation.push_back(A_p.at(i));
            // cout << "GOGO: " << 1 << ", A.at(i): " << A.at(i) << '\n';
            search();
            chosen[i] = false;
